...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Setup cURL to call the MagicAds Trackbox API
    $ch = curl_init('https://mb.magicadsoffers.com/api/signup/procform');

    // Prepare the data for MagicAds API
    $data = array(
        'ai' => '',
        'ci' => '',
        'gi' => '',
        ...
        'password' => 'G7pXr2kQ', // Ensure you handle passwords securely!
        'phone' => getVal($postData, 'phone'),
        'so' => $dynamicSO,
        'cid' => $dynamicCid,
        'sub' => getVal($postData, 'sub') ?? '',
        'ad' => getVal($postData, 'ad') ?? '',
        'term' => getVal($postData, 'term') ?? '',
        'campaign' => getVal($postData, 'campaign') ?? '',
        'medium' => getVal($postData, 'medium') ?? '',
        'lg' => 'EN',
    ...
    if ($isSelfHosted) {
        ...
        saveLead($postData, $getData, $responseArray, 'magicads', 'success', $data);
    ...
    // Set cURL options for the MagicAds API request
    ...
    saveLead($postData, $getData, $responseArray, 'magicads', $leadSaveStatus, $data);
    ...
    if ($httpCode !== 200 || !isset($responseArray['status']) || !$responseArray['status']) {
        // // Return error response if MagicAds API call fails
        // echo json_encode([
        //     'status' => false,
        ...
        // Return success message if MagicAds API call is successful
        // echo json_encode([
        //     'status' => true,
